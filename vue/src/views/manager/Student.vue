  ...
  <div>
    ...
    <el-dialog title="学生信息" :visible.sync="fromVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="form" label-width="100px" style="padding-right: 50px" :rules="rules" ref="formRef">
        ...
        <el-form-item prop="gradeId" label="请选择年级">
          <el-select v-model="form.gradeId" placeholder="请选择所属年级" style="width: 100%">
            <el-option v-for="item in gradeData" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="collegeId" label="请选择学院" >
          <el-select v-model="form.collegeId" placeholder="请选择所属学院" style="width: 100%" @change="getSpeciality">
            <el-option v-for="item in collegeData" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="specialityId" label="请选择专业">
          <el-select v-model="specialityId" placeholder="请选择所属专业" style="width: 100%"  @change="getClasses">
            <el-option v-for="item in specialityData" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="classId" label="请选择班级">
          <el-select v-model="classId" placeholder="请选择所属班级" style="width: 100%">
...
export default {
  ...
  methods: {
    ...
    getSpeciality(collegeId){  //collegeId作为课程选择下拉框的回调参数

      this.$request.get('/speciality/selectAll', {
        params: {
          collegeId: collegeId
        }
      }).then(res => {
    ...
    //   })
    // },
    getClasses(specialityId){  //collegeId作为课程选择下拉框的回调参数

      this.$request.get('/classes/selectAll', {
        params: {
    ...
    loadGrade(){
      this.$request.get('/grade/selectAll').then(res=>{
        if(res.code==='200'){
    ...
    handleEdit(row) {   // 编辑数据
      this.getSpeciality(this.collegeId)
