<template>
  <div>
    <div class="search">
      <el-input placeholder="请输入说明查询" style="width: 200px" v-model="content"></el-input>
    ...
    <div class="table">
      <el-table :data="tableData" stripe  @selection-change="handleSelectionChange"> <!--选中复选框时调用handleSelectionChange方法-->
        ...
        <el-table-column prop="scoreValue" label="总成绩" show-overflow-tooltip></el-table-column>
  ...
  methods: {
    ...
    save() {   // 保存按钮触发的逻辑  它会触发新增或者更新
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.$request({
            url: this.form.id ? '/warn/update' : '/warn/add', //判断表单有没有id，如果有就是编辑，如果没有就是新增
    ...
    del(id) {   // 单个删除
      this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/warn/delete/' + id).then(res => { //把id传给后台 即通过访问warn/delete/id这个url调用通过id删除的方法
    ...
    delBatch() {   // 批量删除
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/warn/delete/batch', {data: this.ids}).then(res => {
          if (res.code === '200') {   // 表示操作成功
            this.$message.success('操作成功')
            this.load(1)
          } else {
    ...
    load(pageNum) {  // 分页查询 通过输入页数来获取分页的数据
      ...
      this.$request.get('/warn/selectPage', { //selectPage是WarnService里的接口
        params: {
