<template>
  <div>
    <div class="search">
      <el-input placeholder="请输入专业名称" style="width: 200px" v-model="name"></el-input>
  ...
  methods: {
    ...
    save() {   // 保存按钮触发的逻辑  它会触发新增或者更新
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.$request({
            url: this.form.id ? '/speciality/update' : '/speciality/add', //判断表单有没有id，如果有就是编辑，如果没有就是新增
    ...
    del(id) {   // 单个删除
      this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/speciality/delete/' + id).then(res => { //把id传给后台 即通过访问speciality/delete/id这个url调用通过id删除的方法
    ...
    load(pageNum) {  // 分页查询 通过输入页数来获取分页的数据
      ...
      this.$request.get('/speciality/selectPage', { //selectPage是SpecialityService里的接口
        params: {
