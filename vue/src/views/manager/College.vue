  ...
  methods: {
    ...
    save() {   // 保存按钮触发的逻辑  它会触发新增或者更新
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.$request({
            url: this.form.id ? '/college/update' : '/college/add', //判断表单有没有id，如果有就是编辑，如果没有就是新增
    ...
    del(id) {   // 单个删除
      this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/college/delete/' + id).then(res => { //把id传给后台 即通过访问college/delete/id这个url调用通过id删除的方法
    ...
    load(pageNum) {  // 分页查询 通过输入页数来获取分页的数据
      ...
      this.$request.get('/college/selectPage', { //selectPage是CollegeService里的接口
        params: {
