...
const routes = [
  {
    ...
    redirect: '/home',  // 重定向到主页 如果输入localhost:8080 会自动重定向到localhost:8080/home
    children: [
      { path: '403', name: 'NoAuth', meta: { name: '无权限' }, component: () => import('../views/manager/403') },
      { path: 'home', name: 'Home', meta: { name: '系统首页' }, component: () => import('../views/manager/Home') },
      { path: 'admin', name: 'Admin', meta: { name: '管理员信息' }, component: () => import('../views/manager/Admin') },
      { path: 'teacher', name: 'Teacher', meta: { name: '教师信息' }, component: () => import('../views/manager/Teacher') }, //对应manager文件夹下的Teacher.vue 即当访问/teacher这个路由时，就医渲染这个vue文件
      ...
      { path: 'attendance', name: 'Attendance', meta: { name: '考勤管理' }, component: () => import('../views/manager/Attendance') },
      { path: 'warn', name: 'Warn', meta: { name: '学业预警' }, component: () => import('../views/manager/Warn') },
      { path: 'analysis', name: 'Analysis', meta: { name: '成绩分析' }, component: () => import('../views/manager/Analysis') }
    ]
  },
  {
    path: '/front',
